	...
	<section class="full-box dashboard-contentPage">
		...
		<nav class="full-box dashboard-Navbar">
			<ul class="full-box list-unstyled text-right">
				...
				<?php
					include("../vista/config/conexion.php");
				?> 
				<!-- --------------NOTIFICACIONES--------------------------->
		...
		<div class="container-fluid">
			...
			<p class="lead">Seccion Donde se muestran todos los clientes registrados en el Gimnasio!</p>
		</div>
					<?php 
...
?>
							...
							<div class="container-fluid">
								<div class="row">
									<div class="col-xs-12 col-md-10 col-md-offset-1">
									    <form action="../vista/cliente/registro.php" method="post" enctype='multipart/form-data' autocomplete="off">
									    	<div class="form-group label-floating">
											  <label class="control-label">Nombres y Apellidos</label>
											  <input name="nomcli" class="form-control" pattern="[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{1,50}" required="" maxlength="50" type="text">
											</div>
											...
											<div class="form-group label-floating">
											  ...
											  <input name="telef" class="form-control" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" maxlength="9" type="text">
											</div>
											<div class="form-group label-floating">
											  <label class="control-label">Contraseña</label>
											  <input name="clave" class="form-control" type="password">
											</div>
	...
	try{	
		...
		foreach ($db->query($sql) as $row) {
			?>
			<tr>
				<td><?php echo $row['id_clien']; ?></td>
			
				<td><?php  echo "<img src='../assets/images/subi_clie/".$row['foto']."'width='90'"; ?></td>
				...
				<td>
						 <?php    if($row['estado']==1)  { ?> 
						  <form  method="get" action="javascript:activo('<?php echo $row['id_clien']; ?>')">
							<button type="submit" class="btn btn-success btn-xs"><b>Activo</b></button>
						  </form>
						<?php  }   else {?> 

						  <form  method="get" action="javascript:inactivo('<?php echo $row['id_clien']; ?>')"> 
						...
						<td>
			<a href="#ver_<?php echo $row["id_clien"]; ?>" title="Ver datos" data-backdrop="false" class="btn btn-primary btn-raised btn-xs" data-toggle="modal"><i class="zmdi zmdi-eye"></i></a>
			
			
			
			<a href="#delete_<?php echo $row["id_clien"]; ?>" title="Eliminar" data-backdrop="false" class="btn btn-danger btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span></a>
			
			
			<a href="#file_<?php echo $row["id_clien"]; ?>" title="Editar" data-backdrop="false" class="btn btn-success btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-picture"></span></a>
			
			
			<a href="#edit_<?php echo $row["id_clien"]; ?>" title="Editar" data-backdrop="false" class="btn btn-warning btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
			
			<a href="#pass_<?php echo $row["id_clien"]; ?>" title="Contraseña" data-backdrop="false" class="btn btn-info btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-lock" aria-hidden="true"></span></a>
			<?php include('../vista/cliente/Modalpassw.php'); ?>
  ...
  reader.onload = function(){
    ...
    preview.append(image);
  	...
  	<script>
	function activo(id_clien)
{
	var id=id_clien;
	$.ajax({
        type:"GET",
		url:"../assets/ajax/editar_estado_activo_cliente.php?id="+id,
    }).done(function(data){
...
function inactivo(id_clien)
{
	var id=id_clien;
	$.ajax({
		type:"GET",
		url:"../assets/ajax/editar_estado_inactivo_cliente.php?id="+id,
    }).done(function(data){
        window.location.href ='cliente.php';
    })
}
	...
	<script type="text/javascript">
      function myFunction() {
        $.ajax({
          url: "../vista/cliente/notificaciones.php",
          type: "POST",
          processData:false,
