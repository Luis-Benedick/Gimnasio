	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles"><i class="zmdi zmdi-shopping-cart zmdi-hc-fw"></i> Productos <small>Productos</small></h1>
			</div>
			<p class="lead">Registro y Listra de todos nuestros productos!</p>
		</div>
					<?php 
							...
							<div class="container-fluid">
								<div class="row">
									<div class="col-xs-12 col-md-10 col-md-offset-1">
									    ...
									    <form action="../vista/producto/registro.php" method="post" autocomplete="off" enctype='multipart/form-data'>
										
									    	<div class="form-group label-floating">
											  <label class="control-label">Nombre del producto</label>
											  <input name="nomprod" class="form-control"  required="" maxlength="50" type="text">
											</div>
											...
											<div class="form-group">
										        ...
										        <select name="id_cate" class="form-control" id="cate">
											...
											<div class="form-group label-floating">
											  ...
											  <input name="descrp" class="form-control" pattern="[a-zA-ZáéíóúÁÉÍÓÚñÑ-._ ]{1,50}" required="" maxlength="255" type="text">
											</div>
											
											<div class="form-group">
											  <label class="control-label">Precio del producto</label>
											  <input name="preciop" class="form-control" maxlength="5" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"  type="text">
											</div>
											
											<div class="form-group">
											  <label class="control-label">Cantidad del producto</label>
											  <input name="canti" class="form-control" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"  type="text">
											</div>
											
											<div class="form-group">
											  <label class="control-label">Fecha </label>
											  <input name="fechare" class="form-control" value="<?php echo date("Y-m-d");?>" type="date">
											</div>
	...
	try{	
		...
		foreach ($db->query($sql) as $row) {
			...
			<tr>
				...
				<td><?php  echo "<img src='../assets/images/subi_produc/".$row['foto']."'width='90'"; ?></td>
			
				<td><?php echo $row['nomprod']; ?></td>
				<td><?php echo $row['nomcate']; ?></td>
				<td>S./<?php echo $row['preciop']; ?></td>
				
				<td>
						 <?php    if($row['estado']==1)  { ?> 
						  <form  method="get" action="javascript:activo('<?php echo $row['id_produc']; ?>')">
							<button type="submit" class="btn btn-success btn-xs"><b>Activo</b></button>
						  </form>
						<?php  }   else {?> 

						  <form  method="get" action="javascript:inactivo('<?php echo $row['id_produc']; ?>')"> 
						...
						<td>
			<a href="#ver_<?php echo $row["id_produc"]; ?>" title="Ver datos" data-backdrop="false" class="btn btn-primary btn-raised btn-xs" data-toggle="modal"><i class="zmdi zmdi-eye"></i></a>
			
			
			<a href="#delete_<?php echo $row["id_produc"]; ?>" title="Eliminar" data-backdrop="false" class="btn btn-danger btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span></a>
			
			<a href="#edit_<?php echo $row["id_produc"]; ?>" title="Editar" data-backdrop="false" class="btn btn-warning btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
			
			<a href="#valorar_<?php echo $row["id_produc"]; ?>" title="Valorar" data-backdrop="false" class="btn btn-secundary btn-raised btn-xs" data-toggle="modal"><i class="zmdi zmdi-star"></i></a>
			
			<?php include('../vista/producto/Modalpassw.php'); ?>
...
<script>
	function activo(id_produc)
{
	var id=id_produc;
	$.ajax({
        type:"GET",
		url:"../assets/ajax/editar_estado_activo_product.php?id="+id,
    }).done(function(data){
...
function inactivo(id_produc)
{
	var id=id_produc;
	$.ajax({
		type:"GET",
		url:"../assets/ajax/editar_estado_inactivo_product.php?id="+id,
    }).done(function(data){
        window.location.href ='producto.php';
    })
}
   ...
   <script src="../assets/js/funciones/functions_cate.js"></script>
...
<script src="../assets/js/funciones/jquery.rating.pack.js"></script>
