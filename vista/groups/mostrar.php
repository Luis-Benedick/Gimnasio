...
<head>
	...
	<link rel="stylesheet" href="../assets/css/hora2.css">
	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles"><i class="zmdi zmdi-balance zmdi-hc-fw"></i> Actividades <small>Grupos</small></h1>
			</div>
			<p class="lead">Registro y Lista de todos los grupos del GYM!</p>
		</div>
					<?php 
...
?>
							...
							<div class="container-fluid">
								<div class="row">
									<div class="col-xs-12 col-md-10 col-md-offset-1">
									    <form action="../vista/groups/registro.php" method="post" autocomplete="off">
									    	<div class="form-group label-floating">
											  <label class="control-label">Nombre del grupo</label>
											  <input name="nomgru" class="form-control" pattern="[a-zA-ZáéíóúÁÉÍÓÚñÑ-_ ]{1,50}" required=""  type="text">
											</div>
											
											<div class="form-group" >
										        <label class="control-label">Salón</label>
										        <select name="id_salon" id="salon" class="form-control">
											...
											<div class="form-group" >
										        ...
										        <select name="id_clien" id="cliente" class="form-control">
											...
											<div class="form-group">
											  ...
											  <input name="fech_entr" value="<?php echo date("Y-m-d");?>" class="form-control" type="date">
											</div>
											
											<div class="form-group">
											  <label class="control-label">Fecha de salida</label>
											  <input name="fech_salid"  class="form-control" type="date">
											</div>
											
											<div class="form-group">
											  <label class="control-label">Hora de entrada</label>
											  <input name="hora_entr" id="hini"  class="form-control" type="text">
											</div>
											
											<div class="form-group">
											  <label class="control-label">Hora de salida</label>
											  <input name="hora_salid" id="hfinal"  class="form-control" type="text">
											</div>
	...
	$db = $database->open();
	try{	
		$sql = 'SELECT groups.id_grup, groups.nomgru, salon.nomsalo, cliente.nomcli, cliente.foto, groups.fech_entr, groups.fech_salid, groups.hora_entr, groups.hora_salid, groups.estado  FROM groups INNER JOIN salon ON groups.id_salon=salon.id_salon INNER JOIN cliente ON groups.id_clien=cliente.id_clien';
		foreach ($db->query($sql) as $row) {
			...
			<tr>
				...
				<td><?php echo $row['nomsalo']; ?></td>
				<td><?php echo $row['nomcli']; ?></td>
				
				<td>
						 <?php    if($row['estado']==1)  { ?> 
						  <form  method="get" action="javascript:activo('<?php echo $row['id_grup']; ?>')">
							<button type="submit" class="btn btn-success btn-xs"><b>Activo</b></button>
						  </form>
						<?php  }   else {?> 

						  <form  method="get" action="javascript:inactivo('<?php echo $row['id_grup']; ?>')"> 
						...
						<td>
			<a href="#ver_<?php echo $row["id_grup"]; ?>" title="Ver datos" data-backdrop="false" class="btn btn-primary btn-raised btn-xs" data-toggle="modal"><i class="zmdi zmdi-eye"></i></a>
			
			
			<a href="#delete_<?php echo $row["id_grup"]; ?>" title="Eliminar" data-backdrop="false" class="btn btn-danger btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span></a>
			
			
			<a href="#edit_<?php echo $row["id_grup"]; ?>" title="Editar" data-backdrop="false" class="btn btn-warning btn-raised btn-xs" data-toggle="modal"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
			<?php include('../vista/groups/Modalpassw.php'); ?>
...
function inactivo(id_cate)
{
   ...
   <script src="../assets/js/funciones/functions_cliente.js"></script>
	<script src="../assets/js/funciones/functions_salon.js"></script>
	

 <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js"></script>
  <script src='../assets/js/hora.js'></script>
  <script src='../assets/js/horaf.js'></script>
   <script src='../assets/js/hora2.js'></script>
</body>
</html>